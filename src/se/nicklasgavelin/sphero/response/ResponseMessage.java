...
import se.nicklasgavelin.sphero.response.InformationResponseMessage.INFORMATION_RESPONSE_CODE;
import se.nicklasgavelin.sphero.response.ResponseMessage.ResponseHeader.RESPONSE_TYPE;
...
public class ResponseMessage
{
		...
		// Information message that we received without doing any command message
			case INFORMATION:
				try
				{
					...
					INFORMATION_RESPONSE_CODE ir = INFORMATION_RESPONSE_CODE.valueOf( data[RESPONSE_CODE_INDEX] );
					...
					ResponseMessage dim = cons.newInstance( rh );
	...
	public enum RESPONSE_CODE
	{
		...
		CODE_OK(
				new int[] { 0, 0 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR, RESPONSE_TYPE.INFORMATION } ), CODE_ERROR_GENERAL(
				new int[] { 1 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_CHECKSUM(
				new int[] { 2 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_FRAGMENT(
				new int[] { 3 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_BAD_COMMAND(
				new int[] { 4 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_UNSUPPORTED(
				new int[] { 5 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_BAD_MESSAGE(
				new int[] { 6 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_PARAMETER(
				new int[] { 7 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_EXECUTE(
				new int[] { 8 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_MAIN_APP_CORRUPT(
				new int[] { 52 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_TIME_OUT(
				new int[] { -1 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), CODE_ERROR_UNKNOWN(
				new int[] { 53 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } ), UNKNOWN_RESPONSE_CODE(
				new int[] { -2 },
				new RESPONSE_TYPE[] { RESPONSE_TYPE.REGULAR } );

		private Map<RESPONSE_TYPE, Byte> codes;
		 ...
		 */
		public Byte getCode( RESPONSE_TYPE type )
		{
		 ...
		 *         found
		 */
		public RESPONSE_TYPE getType( byte code )
		{
			for( RESPONSE_TYPE t : this.codes.keySet() )
				if(codes.get(t).equals(code) )
					return t;
		 ...
		 */
		public static RESPONSE_CODE valueOf( int code, RESPONSE_TYPE type )
		{
		 ...
		 * @return The response code represented by the code
		 */
		public static RESPONSE_CODE valueOf( byte code, RESPONSE_TYPE type )
		{
			return valueOf((int) code, type);
		}
	}
}
