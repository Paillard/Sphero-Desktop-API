...
public class DBusSignal extends Message
{
   ...
   {
      super(Endian.BIG, MessageType.SIGNAL, (byte) 0);

      if (null == path || null == member || null == iface)
         throw new MessageFormatException(getResource("Must specify object path, interface and signal name to Signals."));
   ...
   static DBusSignal createSignal(Class<? extends DBusSignal> c, String source, String objectpath, String sig, long serial, Object... parameters) throws DBusException
   {
      ...
      if (null != c.getEnclosingClass()) {
         ...
         throw new DBusException(getResource("Signals must be declared as a member of a class implementing DBusInterface which is the member of a package."));
   ...
   private static Class<? extends DBusSignal> createSignalClass(String intname, String signame) throws DBusException
   {
      ...
      do {
		...
		if (null == c) 
			throw new DBusException(getResource("Could not create class from signal ")+intname+'.'+signame);
   ...
   protected DBusSignal(String objectpath, Object... args) throws DBusException
   {
      ...
      if (!objectpath.matches(AbstractConnection.OBJECT_REGEX)) throw new DBusException(getResource("Invalid object path: ")+objectpath);
      ...
      if (null == enc ||
       ...
       headers.put(HeaderField.PATH, objectpath);
       headers.put(HeaderField.MEMBER, member);
       headers.put(HeaderField.INTERFACE, iface);

      Vector<Object> hargs = new Vector<>();
      hargs.add(new Object[] { HeaderField.PATH, new Object[] { ArgumentType.OBJECT_PATH_STRING, objectpath } });
      hargs.add(new Object[] { HeaderField.INTERFACE, new Object[] { ArgumentType.STRING_STRING, iface } });
      hargs.add(new Object[] { HeaderField.MEMBER, new Object[] { ArgumentType.STRING_STRING, member } });
      ...
      if (0 < args.length) {
         try {
            ...
            if (AbstractConnection.EXCEPTION_DEBUG && Debug.debug) Debug.print(Debug.ERR, e);
            throw new DBusException(getResource("Failed to add signal parameters: ")+e.getMessage());
         }
      }
