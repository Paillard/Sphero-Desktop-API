...
public class Message
{
    ...
    protected Message(byte endian, byte type, byte flags) throws DBusException
    {
        ...
        this.flags = flags;
        preallocate(4);
        append("yyyy", endian, type, flags, PROTOCOL);
    }
    /**
    ...
    void populate(byte[] msg, byte[] headers, byte... body) throws DBusException
    {
        ...
        this.body = body;
        bufferuse = 3;
        bodylen = ((Number) extract(ArgumentType.UINT32_STRING, msg, 4)[0]).longValue();
        serial = ((Number) extract(ArgumentType.UINT32_STRING, msg, 8)[0]).longValue();
    ...
    protected void appendBytes(byte... buf)
    {
        ...
        if (preallocated > 0) {
            if (paofs +buf.length > pabuf.length)
                throw new ArrayIndexOutOfBoundsException(MessageFormat.format(getResource("Array index out of bounds, paofs={0}, pabuf.length={1}, buf.length={2}."), new Object[] {paofs, pabuf.length, buf.length }));
    ...
    private int appendone(byte[] sigb, int sigofs, Object data) throws DBusException
    {
        try {
            ...
            switch (sigb[i]) {
                ...
                case ArgumentType.OBJECT_PATH:
                    ...
                    try {
                        ...
                        throw new DBusException(getResource("System does not support UTF-8 encoding"));
                    }
                    ...
                    if (data.getClass().isArray() &&
                        ...
                        switch (sigb[i]) {
                            ...
                            case ArgumentType.FLOAT:
                                ...
                                break;
                            default:
                                throw new MarshallingException(getResource("Primative array being sent as non-primative array."));
                        }
                        appendBytes(primbuf);
                        ...
                        for (Entry<Object,Object> o: ((Map<Object,Object>) data).entrySet())
                            diff = appendone(sigb, i, o);
                        if (i == diff) {
                ...
                case ArgumentType.DICT_ENTRY1:
                    ...
                    if (data instanceof Entry) {
                        i++;
                        i = appendone(sigb, i, ((Entry) data).getKey());
                        i++;
                        i = appendone(sigb, i, ((Entry) data).getValue());
                        i++;
                    } else {
                        contents = (Object[]) data;
                        j = 0;
            ...
            throw new MarshallingException(MessageFormat.format(getResource("Trying to marshall to unconvertable type (from {0} to {1})."), data.getClass().getName(), sigb[sigofs]));
        }
    }
    /**
        ...
        switch (sigb[ofs[0]]) {
            ...
            case ArgumentType.ARRAY:
                ...
                if (length > DBusConnection.MAX_ARRAY_LENGTH)
                    throw new MarshallingException(getResource("Arrays must not exceed ")+DBusConnection.MAX_ARRAY_LENGTH);
                // optimise primatives
                switch (sigb[ofs[0]]) {
                    case ArgumentType.BYTE:
                        rv = new byte[length];
    ...
    public Object[] extract(String sig, byte[] buf, int... ofs) throws DBusException
    {
        ...
        byte[] sigb = sig.getBytes();
        for (; ofs[0] < sigb.length; ofs[0]++) {
            rv.add(extractone(sigb, buf, ofs, false));
        }
        return rv.toArray();
    }
    /**
